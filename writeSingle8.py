...
for j in range(repeat):
    ...
    def runTest():
        ...
        for i in range(len(data)):
            tx_data = SPIUtility.writeSingle8(i,data[i])
        ...
        #### readSingle8
        print("==> writeSingle8 readSingle8")
        response = []
        for i in range(len(data)):
        ...
        for i in range( len(response) ):
            if(data[i] != response[i]):
                ...
                print(response[i])
                is_pass = False
                print("Fail writeSingle8 readSingle8")
                break

        #### readBurst8
        print("==> writeSingle8 readBurst8")
        ...
        for i in range( len(response) ):
            if(data[i] != response[i]):
                is_pass = False
                print("Fail writeSingle8 readBurst8")
                break

        #### readSingle32
        print("==> writeSingle8 readSingle32")
        response = []
        ...
        for i in range( len(response) ):
            if(data[i] != response[i]):
                ...
                print(response[i])
                is_pass = False
                print("Fail writeSingle8 readSingle32")
                break
      
        #### readImage
        print("==> writeSingle8 readImage")
        response = []
